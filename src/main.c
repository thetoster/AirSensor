...
struct UARTFrame {
  ...
  __packed uint8_t   quality;
  __packed uint8_t   sensorId;
};

//Pin mapping:
//Button: PA-0
//UART PINS: TX-PA2, RX=PA3
//Sensor 1: PA1
//Sensor 2: PC4
...
int main(void) {
  ...
  AirSensing.CalibrationValue = 0;
  CoSensing.CallibrationSamples = 10;
  CoSensing.CalibrationValue = 0;
...
//DEBUG
//    char s[100];
//    sprintf(s, "a=%d (%d), q=%d   a[0]=%d, a[1]=%d, a[2]=%d\n\r",
//        (int)AirSensing.CurrentValue, (int)AirSensing.CalibrationValue, (int)AirSensing.Quality,
//        (int)AirSensing.AirLevels[0], (int)AirSensing.AirLevels[1], (int)AirSensing.AirLevels[2]);

    //PRODUCTION:
    struct UARTFrame frame;
    frame.quality = AirSensing.Quality;
    frame.value = AirSensing.CurrentValue;
    frame.sensorId = 0;
    HAL_UART_Transmit(&huart2, (uint8_t*)(&frame), sizeof(frame), 50);

    frame.quality = CoSensing.Quality;
    frame.value = CoSensing.CurrentValue;
    frame.sensorId = 1;
    HAL_UART_Transmit(&huart2, (uint8_t*)(&frame), sizeof(frame), 50);
